...
class MyArticle extends React.Component {
    ...
    handleOnDel = (flag, id) => {
        ...
        let failedMsg = ''
        if (flag === 'on') {
            url = `/v1/wyw/article/stopOrStart/${id}/0`
            successMsg = '启用成功'
            failedMsg = '启用失败'
        } else if (flag === 'off') {
            url = `/v1/wyw/article/stopOrStart/${id}/1`
            successMsg = '禁用成功'
            failedMsg = '禁用失败'
        }
        request(url, {
    ...
    * */ 
   getBuilding=()=>{
       let url= '/v1/wyw/article/selectAllByPage?pageNo='+this.state.current+'&pageSize=' + this.state.pageSize
       request(url,{
           method: 'GET'
       }).then((res)=>{
           if(res.message=='成功'){
...
export default Form.create()(MyArticle);